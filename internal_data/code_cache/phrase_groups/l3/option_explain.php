...
return array (
  ...
  'option_explain.boardUrl' => 'URL chính cho diễn đàn của bạn. Không bao gồm dấu "/" hoặc "index.php". URL trông giống như sau: https://www.codevn.net/forums',
...
This option requires that the specified bounce account is a catch-all account or supports a "+" as a wildcard separator (such as in Gmail). For example, if this option is enabled with a bounce address of bounce@example.com, the email might be returned to bounce+123abc+user=domain.com@example.com.',
  ...
  'option_explain.imageLinkProxyLogLength' => 'Tùy chọn này kiểm soát khoảng thời gian mà các bản ghi proxy sẽ được duy trì sau khi yêu cầu cuối cùng được thực hiện cho một hình ảnh hoặc một liên kết. Nếu một mục nhập không được yêu cầu cho khoảng thời gian này, dữ liệu của nó (bao gồm thời gian yêu cầu đầu tiên và tổng số lượt truy cập) sẽ bị xóa. Các bản ghi proxy của hình ảnh sẽ không bao giờ bị xóa trừ khi dữ liệu hình ảnh đã được xoá (Image Cache Lifetime). Sử dụng 0 để vô hiệu hóa tỉa cành của các bản ghi.',
...
/*$(function()
{
  ...
  'option_explain.showFirstCookieNotice' => 'Khi thành viên xem trang đầu tiên (không có cookie XenForo), nếu được bật, thành viên sẽ thấy thông báo cho biết cookie được sử dụng bởi trang này và tiếp tục sử dụng đồng ý với các cookie. Tùy chọn này được thiết kế để làm cho việc tuân thủ các quy tắc về cookie của EU dễ dàng hơn.',
