...
<div class="block">
		...
		if ($__templater->isTraversable($__vars['purchased'])) {
			foreach ($__vars['purchased'] AS $__vars['upgrade']) {
				...
				if ($__vars['active']['end_date']) {
					$__compilerTemp4 .= '
											' . 'Hết hạn' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->fn('date_dynamic', array($__vars['active']['end_date'], array(
					))) . '
										';
				} else {
					$__compilerTemp4 .= '
	...
	if (strlen(trim($__compilerTemp1)) > 0) {
		...
		$__finalCompiled .= '
			<div class="block-row">' . 'Hiện tại không có nhóm thành viên có thể mua để nâng cấp.' . '</div>
		';
	}
	$__finalCompiled .= '
	</div>
