...
return array('macros' => array('moderator_menu_actions' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canViewWarnings', array()) AND ($__vars['user']['warning_count'] AND ($__vars['context'] == 'tooltip'))) {
		$__finalCompiled .= '
		<a href="' . $__templater->fn('link', array('members', $__vars['user'], ), true) . '#warnings" class="menu-linkRow">' . 'Xem cảnh cáo (' . $__templater->filter($__vars['user']['warning_count'], array(array('number', array()),), true) . ')' . '</a>
	';
	}
	$__finalCompiled .= '
	';
	if ($__templater->method($__vars['xf']['visitor'], 'canViewIps', array())) {
		...
		<a href="' . $__templater->fn('link', array('members/shared-ips', $__vars['user'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'IP chia sẻ' . '</a>
	';
	}
	$__finalCompiled .= '
	';
