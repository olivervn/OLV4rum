	...
	if ($__vars['profilePost']['user_id'] == $__vars['profilePost']['profile_user_id']) {
		...
		if ($__vars['profilePost']['user_id'] == $__vars['xf']['visitor']['user_id']) {
			...
			$__finalCompiled .= '
				...
				' . '' . $__templater->fn('username_link', array($__vars['liker'], false, array('defaultname' => $__vars['fallbackName'], ), ), true) . ' đã thích <a ' . (('href="' . $__templater->fn('link', array('profile-posts', $__vars['profilePost'], ), true)) . '"') . '>bài viết của ' . $__templater->escape($__vars['profilePost']['username']) . '</a> trên hồ sơ của bạn.' . '
			';
		} else {
			$__finalCompiled .= '
